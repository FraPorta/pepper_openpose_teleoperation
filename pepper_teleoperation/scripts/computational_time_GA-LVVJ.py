...
class PlotAngles:
    ...
    def run(self):
        ...
        difference = []
        for time_init, time_end  in zip(timestamp_init, timestamp_end):
            t1 = datetime.strptime(time_init, '%H:%M:%S.%f')
            t2 = datetime.strptime(time_end, '%H:%M:%S.%f')
            difference.append(t2-t1)   
