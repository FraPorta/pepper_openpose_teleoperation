...
try:
        ...
        if len(corners) > 0:
            ...
            for (markerCorner, markerID) in zip(corners, ids):
                ...
                if not markerID in found_ids:
                    ...
                    timestamp = date_now.strftime('%H:%M:%S.%f')
                    ...
                    marker_info['time_elapsed_blue']    = date_now - now
                    marker_info['time_elapsed_green']   = None
                ...
                # Counter for changing color
                if count[str(markerID)] > 30:
                    highlight[str(markerID)] = False
                    count[str(markerID)] = 0
                    # Save the time when the marker is confirmed (turns green)
                    time_elapsed[str(markerID)] = datetime.now() - now
    ...
    for key in markers_list.keys():
        ...
        markers_list[key]['time_elapsed_green'] = time_elapsed[str(key)]
